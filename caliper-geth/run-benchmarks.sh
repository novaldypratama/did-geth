...
echo "Checking if Geth node is ready..."
node check-geth.js
if [ $? -ne 0 ]; then
  echo "Geth node is not ready. Please make sure it's running and synced."
  exit 1
fi

# Bind Caliper to Ethereum
echo "Binding Caliper to Geth..."
...
caliper launch manager \
  ...
  --caliper-networkconfig networks/ethereum/geth-network.json \
  --caliper-flow-only-test \
  --caliper-report-name "ssi-geth-clique-benchmark-$(date +%Y%m%d-%H%M%S)" \
